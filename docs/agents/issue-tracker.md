...
Issues and specs for this repo live as GitHub issues on `TristanyJacoby/wellsync-mobile`. Use the `gh` CLI for all operations.

> Note: this working directory isn't a git repo yet (no `.git`, no remote), and the `gh` CLI isn't installed locally. Before these conventions work, run `git init && git remote add origin https://github.com/TristanyJacoby/wellsync-mobile.git` and install `gh` (`brew install gh` on macOS), then `gh auth login`.
